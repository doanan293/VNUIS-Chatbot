...
Phat_am = {
    ...
    " VCS ": " vi si ét ",
    " vcs ": " vi si ét ",
    "/QĐ-TTg": " theo quyết định của thủ tướng chính phủ",
    # Đọc phần mềm
    "ATS.NET": "ây ti ét chấm nét",
    "RMS.NET": "a em ét chấm nét",
    "PRM.NET": "pi a em chấm nét",
    "GFS.NET": "gi ép ét chấm nét",
    "KT-XH": "kinh tế xã hội",
    "DTTS": "dân tộc thiểu số",
    # Viết tắt
    "/QH": " theo quốc hội ",
    "/CP": " theo chính phủ ",
    "/NĐ-CP": " theo nghị định của chính phủ",
    "/NQ": " theo nghị quyết ",
...
Phat_am = dict(
    sorted(list(Phat_am.items()), key=lambda item: len(item[0].strip()), reverse=True)
)
...
Dich_am = {
    ...
    "theo quốc hội số ": "QH",
    " của quốc hội số ": "QH",
    "theo nghị định của chính phủ ": "NĐCP",
    "nợ tài khoản": "\nNợ TK:",
    "có tài khoản": "\n______Có TK:",
    " phẩy": ",",
    " 2 chấm": ":",
...
# Sắp xếp từ lớn đến bé tránh trường hợp thay thế chuỗi con
Dich_am = dict(
    sorted(list(Dich_am.items()), key=lambda item: len(item[0].strip()), reverse=True)
)
Chu_cai = dict(
    sorted(list(Chu_cai.items()), key=lambda item: len(item[0].strip()), reverse=True)
)
...
def Check_dvi_giam_dan_hoac_chi_1dvi(lst: list):
    ...
    for v in lst:
        if v in donvi:
            if (
                dvi_truoc == None
            ):  # nếu donvi hiện tại < donvi trước thì tiếp tục duyệt tìm
                dvi_truoc = donvi[v]
...
def Loc_lst_num_va_sap_Xep(words: str) -> list:
    ...
    for v in words.split(" "):
        if v in number_dac_biet and not num:
            continue
        if v in number or v in donvi:
            ...
            if len(num) == 1 and num[0] in [
                "ba",
                "chín",
            ]:  # Do dùng replace nên tránh TH chữ ví dụ 'bao' -> '3o'
                num = []
            else:
                lst_num += [num]
                num = []
...
def Xu_ly_DMY_cho_text(words: str):
    ...
    dmy: list[str] = []
    check = False
    thutu_dmy = {"ngày": "tháng", "tháng": "năm", "năm": "ngày", "mùng": "tháng"}
    dmy_ketiep = ""
    for v in words.split(" "):
    ...
    result = dict(
        sorted(
            list(result.items()), key=lambda item: len(item[0].strip()), reverse=True
        )
    )
...
def Xu_ly_text(words: str) -> str:
    ...
    for i in range(len(words)):
        try:
            if words[i] == "5" and (
                words[i - 8 : i - 3] == "tháng" or words[i - 7 : i - 2] == "tháng"
            ):
